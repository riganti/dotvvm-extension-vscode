...
const worldToSvelte = [ { text: 'svelte', range: { start: { line: 0, character: 7 }, end: { line: 0, character: 12 } } } ]
describe('Document', () => {
    it('gets the correct text', () => {
        const document = new DotvvmDocument('file:///hello.svelte', '<h1>Hello, world!</h1>');
    ...
    it('increments the version on edits', () => {
        const document = new DotvvmDocument('file:///hello.svelte', 'hello');
        assert.strictEqual(document.version, 0);

        document.setText('Hello, world!');
        assert.strictEqual(document.version, 1);
        document.applyChanges(worldToSvelte);
        assert.strictEqual(document.version, 2);
        assert.strictEqual("Hello, svelte!", document.content)
    });

    // it('recalculates the tag infos on edits', () => {
    //     const document = new DotvvmDocument('file:///hello.svelte', '<script>a</script><style>b</style>');
    ...
    it('returns null for non file urls', () => {
        const document = new DotvvmDocument('ftp:///hello.svelte', 'hello');

        assert.strictEqual(document.getFilePath(), null);
    });

    it('gets the text length', () => {
        const document = new DotvvmDocument('file:///hello.svelte', 'Hello, world!');
    ...
    it('supports empty contents', () => {
        const document = new DotvvmDocument('file:///hello.svelte', '');
