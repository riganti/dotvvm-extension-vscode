// From https://github.com/tree-sitter/tree-sitter-html
// Licensed under The MIT License (MIT), Copyright (c) 2014 Max Brunsfeld

// parses the HTML stuff (tags, comments
...
struct Scanner {
  ...
  bool scan_dotvvm_comment(TSLexer *lexer) {
    if (lexer->lookahead != '-') return false;
    lexer->advance(lexer, false);
    if (lexer->lookahead != '-') return false;
    lexer->advance(lexer, false);
    ...
    while (lexer->lookahead) {
      switch (lexer->lookahead) {
        ...
        case '>':
          if (percent) {
            lexer->result_symbol = DOTVVM_COMMENT;
            lexer->advance(lexer, false);
            lexer->mark_end(lexer);
            return true;
          }
  ...
  bool scan(TSLexer *lexer, const bool *valid_symbols) {
    ...
    switch (lexer->lookahead) {
      case '<':
        ...
        if (lexer->lookahead == '%' && valid_symbols[DOTVVM_COMMENT]) {
          lexer->advance(lexer, false);
          return scan_dotvvm_comment(lexer);
        }
...
void *tree_sitter_dotvvm_external_scanner_create() {
  return new Scanner();
}

bool tree_sitter_dotvvm_external_scanner_scan(void *payload, TSLexer *lexer,
                                            const bool *valid_symbols) {
  Scanner *scanner = static_cast<Scanner *>(payload);
  return scanner->scan(lexer, valid_symbols);
}

unsigned tree_sitter_dotvvm_external_scanner_serialize(void *payload, char *buffer) {
  Scanner *scanner = static_cast<Scanner *>(payload);
  return scanner->serialize(buffer);
}

void tree_sitter_dotvvm_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  Scanner *scanner = static_cast<Scanner *>(payload);
  scanner->deserialize(buffer, length);
}

void tree_sitter_dotvvm_external_scanner_destroy(void *payload) {
  Scanner *scanner = static_cast<Scanner *>(payload);
  delete scanner;
}
