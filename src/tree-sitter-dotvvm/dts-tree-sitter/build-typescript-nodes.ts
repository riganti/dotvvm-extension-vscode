// From https://github.com/asgerf/dts-tree-sitter
// MIT License, Copyright 2020 Asger Feldthaus

// + Slight edits to improve the codegen and fix it on current TS version
...
function toCamelCase(str: string) {
    return str.replace(/_[a-zA-Z]/g, t => t.substring(1).toUpperCase())
}

function toCapitalCase(str: string) {
    return toCamelCase(str.replace(/^[a-z]/, t => t.toUpperCase()))
...
function getTypeExprFromRef(ref: NodeTypeRef, index: IndexedData) {
    ...
    if (!ref.named) {
        let arg = JSON.stringify(ref.type);
...
function generateTypeEnum(json: NodeTypeEntry[], { }: IndexedData, printer: Printer) {
    printer.
...
function generateInterfaceFromEntry(entry: NodeTypeEntry, index: IndexedData, printer: Printer) {
    ...
    printer
        ...
        .forEachInRecord(entry.fields, (field, children) => {
            let fieldName = toCamelCase(field) + 'Node';
